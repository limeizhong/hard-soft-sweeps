...
def main_h12(file1,file2,file3,file4,step=0):
    geno=read_imputed_012_file(file1)
    gene_sub=gene_index_file(file2)
    f1_out=open(file3,"wt")
    f2_out=open(file4,"wt")
    for key in gene_sub.keys():
        ...
        if gene_ed != gene_st:
            ...
            print(key,wh[0],wh[1],lh[0],lh[1],ih[0],ih[1],file=f1_out)
            for k in keys:
                print(key,k,whap[k],lhap[k],ihap[k],file=f2_out)
              
if __name__ == '__main__': 
    main_h12(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5]) 

# python h12.py Gm01_012_geno Gm01_gene.index Gm01.h12.stats Gm01.h12.hap 0